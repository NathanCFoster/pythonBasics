...
function updateInventory(newInv, currInv) {
    var expected = [];
    for (let index = 0; index < newInv.length; index++) {
        expected.push(newInv[index]);
    }
    for (let x = 0; x < currInv.length; x++) {
        expected.push(currInv[x]);
        
    }
    for (let index = 0; index < expected.length; index++) {
        for (let x = 0; x < expected.length; x++) {
            if (Object.keys(expected[index]) == Object.keys(expected[x]) && index != x) {
                expected[index][Object.keys(expected[index])] += Object.values(expected[x]);
                console.log("Dupes")
            }
        }
    }
