...
class User:
    ...
    def validateUser(form):
        ...
        if not password_regex.match(form["password"]):
            flash('Password must be 5 characters, have one uppercase letter, one lowercase letter, and one number', 'password')
            isValid = False
        if not email_regex.match(form['email']):
            flash('Must be a valid email', 'email')
            isValid = False
        if form['password'] != form['confirmedPass']: 
            flash("Password must be the same", 'confirmedPass')
        return isValid
    ...
    def newUser(cls, data):
        ...
        newUser = connectToMySQL("users_schema").query_db(query, data)
        return newUser
    
    @classmethod
    def showUsers(cls):
        query = "SELECT * FROM user"
        allUsers = connectToMySQL("users_schema").query_db(query)
        users = []
        for row in allUsers:
    ...
    def currentUser(cls, id):
        ...
        currentUser = connectToMySQL("users_schema").query_db(query)
