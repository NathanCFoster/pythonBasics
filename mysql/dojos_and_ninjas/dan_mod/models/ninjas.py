from dan_mod.config.mysqlconnection import connectToMySQL

class Ninjas:
    def __init__(self, data):
        self.id = data['id']
        self.first_name = data['first_name']
        self.last_name = data['last_name']
        self.age = data['age']
        self.created_at = data['created_at']
        self.updated_at = data['updated_at']
        self.dojos_id = data['dojos_id']

    @classmethod
    def addNinja(cls, data):
        query = "INSERT INTO ninjas (first_name, last_name, age, dojos_id) values (%(first_name)s, %(last_name)s, %(age)s, %(dojos_id)s);"
        newNinja = connectToMySQL("dojos_and_ninjas").query_db(query, data)
        return newNinja

    @classmethod
    def showNinjas(cls, dojo_id):
        query = "select * from dojos left join ninjas on dojos.id = ninjas.dojos_id where dojos.id = %s;" % (dojo_id)
        results = connectToMySQL("dojos_and_ninjas").query_db(query)
        allNinjas = []
        for row in results:
            allNinjas.append(cls(row))
        return allNinjas
